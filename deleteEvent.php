...
if($validForm)
{
	...
	//$eventTime = date("h:i:00");
		...
		$sql = "DELETE FROM final_event WHERE event_id = $recID";
		...
		$statement->bindParam(':eventId', $recID);

		$statement->execute();

		$message = "The Event you have create was deleted successfully.";
	} //end of try
...
else
{
	...
	$sql = "SELECT event_name, event_description, event_date, event_time, event_location FROM final_event WHERE event_id = $recID";
...
<div class="container">
<form id="deleteEvents" name="deleteEvent" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?recID=$recID"; ?>">
  ...
  <p>
    <input type="submit" name="submit" id="button3" value="Delete Event">
